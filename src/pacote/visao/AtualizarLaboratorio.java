...
import pacote.DAO.DAOLaboratorio;
 ...
 * @author Ligia
 */
public class AtualizarLaboratorio extends javax.swing.JFrame {

    /**
     * Creates new form Dados
     */
    public AtualizarLaboratorio() {
        initComponents();
        DAOLaboratorio dadosLaboratorio = new DAOLaboratorio();
        ArrayList<Laboratorio> listaLaboratorios = new ArrayList();
        listaLaboratorios = dadosLaboratorio.selecionarTodosRegistros();
        ...
        for(Laboratorio laboratorio : listaLaboratorios)
        {
            Object[] dados = {
                laboratorio.getId(), 
                laboratorio.getNome(),
                converteDataString(laboratorio.getDataEntrega()), 
                laboratorio.getEspecialidade(), 
                laboratorio.getCnpj()};
            modelo.addRow(dados);
        }
    }
    ...
    private void tabelaDadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabelaDadosMouseClicked
        ...
        if(resposta == 0)
        {
            ...
            laboratorio.setId(Integer.parseInt(tabelaDados.getValueAt(linha, 0).toString()));
            laboratorio.setNome(tabelaDados.getValueAt(linha, 1).toString());
            laboratorio.setDataEntrega(converteStringDate(tabelaDados.getValueAt(linha, 2).toString()));
            laboratorio.setEspecialidade(tabelaDados.getValueAt(linha, 3).toString());
            laboratorio.setCnpj(Integer.parseInt(tabelaDados.getValueAt(linha, 4).toString()));
            new AtualizarFormulario(laboratorio).setVisible(true);
            dispose();
        }
   ...
   //método que converte date (util) em string com formato correta para mostrar na tabela
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AtualizarLaboratório.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AtualizarLaboratório().setVisible(true);
            }
        });
    }
